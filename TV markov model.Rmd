...
## removing empty rows
aangQuotes <- aangQuotes[aangQuotes$quote != "",]


## allocate tons of rows now for efficiency
totalWords <- sum(sapply(aangQuotes[,2], function(x) length(unlist(strsplit(x, " ")))))
aangTrigram <- as.data.table(matrix(ncol = 4, nrow = totalWords+2000))

colnames(aangTrigram) <- c("w1", "w2", "w3", "counts")
aangTrigram <- aangTrigram[, w1:=as.character(w1)]
aangTrigram <- aangTrigram[, w2:=as.character(w2)]
aangTrigram <- aangTrigram[, w3:=as.character(w3)]
aangTrigram <- aangTrigram[, counts:=as.integer(counts)]

count <- 1L
for(i in 1:nrow(aangQuotes)){
  ## split into tokens
  tokens <- unlist(strsplit(aangQuotes[i,2], " ", perl=T))
  ...
  for(j in 1:(length(tokens)-2)){
    ## check if trigram already exists in data table
    
    set(aangTrigram, count, names(aangTrigram), list(tokens[j], tokens[j+1], tokens[j+2], 1))
    count = count + 1L
  }
